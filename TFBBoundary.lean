/-
lake exe tfb_boundary — generate a standalone file whose trust boundary is a
set of package roots rather than a single prefix.

Usage (from the downstream project directory):
  lake exe tfb_boundary --root <Pkg> [--root <Pkg> ...]
...
def usage : String :=
  "Usage: lake exe tfb_boundary --root <Pkg> [--root <Pkg> ...] \
   [--exclude <Prefix> ...] [--env <Module>] <targetDecl> <outputPath>"
